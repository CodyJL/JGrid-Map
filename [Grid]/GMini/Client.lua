...
W = (1/1600)*Display.Width
H = (1/1600)*Display.Width
...
Minimap.Width = ((259)*heightA)+((getElementData(localPlayer,'Mini Map Width') or 0)*heightA) 
Minimap.Height = ((139)*heightA)+((getElementData(localPlayer,'Mini Map Height') or 0)*heightA)
...
addEvent( "GraphicsChange", true )
addEventHandler( "GraphicsChange", root, Minamap )
...
Bigmap = {};
Bigmap.Width, Bigmap.Height = 1278*W, 551*H
Bigmap.PosX, Bigmap.PosY = 159*W, 200*H
Bigmap.IsVisible = false;
Bigmap.CurrentZoom = 2;
Bigmap.MinimumZoom = 0.2;
Bigmap.MaximumZoom = 5;

 --dxDrawRectangle(157*W, 763*H, 1282*W, 45*H, tocolor(0, 0, 0, 218), false)-- Map Size
...
addEventHandler('onClientResourceStart', resourceRoot,
	function()
		...
		if (Minimap.MapTexture) then
			dxSetTextureEdge(Minimap.MapTexture, 'border', tocolor(Minimap.WaterColor[1], Minimap.WaterColor[2], Minimap.WaterColor[3], 0));
		end
	end
);
	...
	function(key, state) 
		if (state) then
			if (key == 'F11') then
				cancelEvent();
				showCursor(false);
				setElementData(localPlayer,'MapVisible',not getElementData(localPlayer,'MapVisible'))
...
addEventHandler('onClientRender', root,
	...
	Minimap.IsVisible = not getElementData(localPlayer,'MapVisible')
	Bigmap.IsVisible = getElementData(localPlayer,'MapVisible')
	
	showChat(Minimap.IsVisible);
			...
			--	dxDrawRectangle(0, 0, Minimap.BiggerTargetSize, Minimap.BiggerTargetSize, tocolor(Minimap.WaterColor[1], Minimap.WaterColor[2], Minimap.WaterColor[3], Minimap.Alpha), false);
				...
				if not getElementData(player,"dontshow") then 
					...
					if (localPlayer ~= player and streamDistance * Minimap.CurrentZoom) then
						...
						local playerR, playerG, playerB = unpack({getPlayerNametagColor(player)})
				...
				--> Draw fully minimap
				dxSetRenderTarget();
				dxDrawImageSection(Minimap.PosX, Minimap.PosY, Minimap.Width, Minimap.Height, Minimap.NormalTargetSize / 2 + (Minimap.BiggerTargetSize / 2) - (Minimap.Width / 2), Minimap.NormalTargetSize / 2 + (Minimap.BiggerTargetSize / 2) - (Minimap.Height / 2), Minimap.Width, Minimap.Height, Minimap.RenderTarget, 0, -90, 0, tocolor(255, 255, 255, 200));
